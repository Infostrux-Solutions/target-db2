...
class DB2TypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_unicode(self, type_, **kw):
        check_server = DB2Dialect.serverType
        return (
            (self.visit_VARGRAPHIC(type_, **kw) + " CCSID 1200")
            if check_server == "DB2"
            else self.visit_VARGRAPHIC(type_, **kw)
        )
...
class DB2DDLCompiler(compiler.DDLCompiler):
    ...
    def get_server_version_info(dialect):
        ...
        return (
            [int(ver_token) for ver_token in dialect.dbms_ver.split(".")[0:2]]
    ...
    def get_column_specification(self, column, **kw):
        ...
        if column.table is not None and column is column.table._autoincrement_column:
            col_spec.extend(["GENERATED BY DEFAULT", "AS IDENTITY", "(START WITH 1)"])
    ...
    def visit_drop_constraint(self, drop, **kw):
        ...
        return (
            ("DROP %s%s" % (qual, const))
            if hasattr(constraint, "uConstraint_as_index")
...
class DB2Dialect(default.DefaultDialect):
    ...
    def initialize(self, connection):
        ...
        if self.dbms_name == "AS":
            ...
            self.dbms_name is None
            or "DB2/" in self.dbms_name
            or "IDS/" in self.dbms_name
        ):
            _reflector_cls = ibm_reflection.DB2Reflector
